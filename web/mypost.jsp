...
<html>
    ...
    <body>
                ...
                <div class="row mt-2">
                    ...
                    <div class="col-md-3">
                        <div class="list-group">
                            ...
                            <!--<button type="button" onclick="window.location.href = 'profile.jsp';"  class="c-link list-group-item list-group-item-action active" aria-current="true">
                                    All Post
                            </button>-->


                            <button type="button" onclick="getPost(0,<%=user.getId()%>, this)" class="c-link list-group-item list-group-item-action active" aria-current="true">
                                My Post
                            </button>
                            ...
                            %>
                            <a href="#" onclick="getPost(<%=c.getCid()%>,<%=user.getId()%>, this)" class="c-link list-group-item list-group-item-action"><%= c.getName()%></a>
                            <%
                                }
                            %>
                        </div>
        ...
        <script>
            function getPost(catId, userId, temp) {
                $("#loader").show();
                $("#post-container").hide();
                $(".c-link").removeClass('active');
                $.ajax({
                    url: "mypost_load.jsp",
                    data: {cid: catId, userId: userId}, // pass userId
                    success: function (data, textStatus, jqXHR) {
                        $('#loader').hide();
                        $('#post-container').show();
            ...
            $(document).ready(function () {
                let allPostRef = $('.c-link')[0];  // First element in the .c-link class
                let userId = <%= user.getId()%>;  // Get the user ID from the session or JSP
                getPost(0, userId, allPostRef); // Pass userId correctly
            });
        </script>
